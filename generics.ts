...
//To start off, let's do the "hello world" of generics: the identity function. The identity function is a
//function that will return back whatever is passed in. You can think of this in a similar way to the
//echo command.
...
//or, we could describe the indentity function using the any type:

function iidentity(arg: any): any {
    return arg;
}

//While using any is certainly generic in that it will cause the function to accept any and all types for
//the type of arg , we actually are losing the information about what that type was when the function
...
//We've now added a type variable Type to the identity function. This Type allows us to capture the
//type the user provides (e.g. number ), so that we can use that information later. Here, we use Type
//again as the return type. On inspection, we can now see the same type is used for the argument and
//the return type. This allows us to traffic that type information in one side of the function and out the
//other.
//We say that this version of the identity function is generic, as it works over a range of types.
//Unlike using any , it's also just as precise (i.e., it doesn't lose any information) as the first
...
let output = identitty<string>("myString");
//let output: string
...
let outtput = identitty("myString");

//let output: string

//Notice that we didn't have to explicitly pass the type in the angle brackets ( <> ); the compiler just
//looked at the value "myString" , and set Type to its type. While type argument inference can be a
//helpful tool to keep code shorter and more readable, you may need to explicitly pass in the type
//arguments as we did in the previous example when the compiler fails to infer the type, as may
