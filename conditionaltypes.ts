...
//We can then use that conditional type to simplify our overloads down to a single function with no
//overloads

function creatteLabel<T extends number | string>(idOrName: T): NameOrId<T>{
    throw "unimplemented";
}

let a = creatteLabel("typescript");
    //let a: NameLabel

let b = creatteLabel(2.8);
    //let b: IdLabel

let c = creatteLabel(Math.random() ? "hello" : 42);
